...
pub async fn setup_game_dev_api() {
    ...
    let app = Router::new()
        .route(&format!("{route_prefix:?}/healthcheck"), get(hello_world_handler))
        .route(&format!("{route_prefix:?}/leaderboard"), post(set_leaderboard).get(get_leaderboard))
        .route(&format!("{route_prefix:?}/save-data"), post(set_save_data).get(get_save_data));
